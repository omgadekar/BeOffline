  ...
  protected void inject(Context context) {
    if (!injected) {
      synchronized (injectedLock) {
        if (!injected) {
          ((ScheduleAlarmReceiver_GeneratedInjector) BroadcastReceiverComponentManager.generatedComponent(context)).injectScheduleAlarmReceiver(UnsafeCasts.<ScheduleAlarmReceiver>unsafeCast(this));
          injected = true;
        }
      }
    }
